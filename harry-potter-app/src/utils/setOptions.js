...
export default function setOptions(books, characters) {
    // this function returns an object to contain the options for the search parameters in the form of {attribute: [option, option, option]}

    // set up the object to hold the options for each attribute
    // the house entries in the character data have other entries that are not related to Hogwarts like "House Black." These entries are ignored for the purpose of this page, so the list is not generated by iterating over the set of characters like the other attributes will be. The list is manually set to only include Hogwarts houses.
    // the species and ancestry entries in the data contain multiples so these are instantiated as sets to avoid dealing with duplicates as the characters are iterated over.
    // the book data does not contain duplicates so it can be instantiated as an array.
    let myOptions = {
    ...
    for (const thisCharacter of characters) {
        ...
        if (thisSpecies) {
            // test against each regex to see if the species needs to be replaced with a key value. If it does, add the corresponding replacement string to the set and set the addedSpecies boolean to true
            ...
            // if the addedSpecies boolean is still false then add the original string to the set
    ...
    // fill out the book-based option set with the titles of the books.
